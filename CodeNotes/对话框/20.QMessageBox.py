...
class MainWindow(QWidget):
    ...
    def showDialog(self):
        text = self.sender().text()  # 再次明确，调用槽函数可能是很多控件、通过sender()来判断
        if text == '显示关于对话框':
            QMessageBox.about(self, "关于", "这是一个关于对话框")
        # 返回值是不同按钮的ID值
        elif text == '显示消息对话框':
            QMessageBox.information(self, "消息", "这是一个消息对话框", QMessageBox.Yes | QMessageBox.No)  # 同时设置不同按钮类型
        elif text == '显示警告对话框':
